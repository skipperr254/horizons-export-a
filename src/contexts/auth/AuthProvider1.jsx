...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!authListenerRef.current) {
          const {
            ...
            handleAuthStateChange(event, session, mountedRef, lastSessionRef, {
              setIsNewUser,
              loadProfile,
              setUser,
              setAuthError,
  ...
  const login = useCallback(
    (email, password) => loginUser(email, password, setAuthError),
    []
  );
  const register = useCallback(
    (name, email, password) =>
      registerUser(name, email, password, setAuthError),
    []
  );
  const signInWithGoogle = useCallback(
    () => signInWithGoogleUser(setAuthError),
    []
  );
  const logout = useCallback(
    () => logoutUser(mountedRef, setUser, setAuthError, lastSessionRef),
    []
  );
  const updateUser = useCallback(
    (newUserData) =>
      updateUserProfile(newUserData, user, setUser, setAuthError),
    [user]
  );
